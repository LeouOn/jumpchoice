...
- Synced with upstream `Pasta-Devs/Marinara-Engine` v1.6.1 (commit `65a0395e` "Merge branch 'staging' into main"). Brings 229 upstream commits into JumpChoice while preserving all fork-specific work (CYOA editor, Jumpchain character building, 4-agent system, memory Tier 2, OCR, zhipu/minimax/deepseek providers, generate.routes.ts service architecture refactor, narrative engine).
- Bumped version to 1.7.0 (minor version bump over upstream's v1.6.1 to signal fork's additional features).
