# JumpChoice Module Specifications (Future Vision)

> **⚠️ STATUS: ASPIRATIONAL — NOT CURRENT REALITY**
>
> This document specifies modules from the original fork plan, most of which
> are **not yet implemented**. It uses `pgTable` Postgres schemas while the
> actual codebase is SQLite/file-backed (`drizzle-orm/sqlite-core`).
>
> Modules documented here that DO NOT exist as packages yet:
> - `@jumpchoice/memory` — memory features live inside `packages/server/`
> - `@jumpchoice/vision` — vision pipeline lives inside `packages/server/`
> - `@jumpchoice/build-lab` — not yet implemented (see `ROADMAP.md` Phase 4)
> - `@jumpchoice/campaign` — not yet implemented (see `ROADMAP.md` Phase 4)
> - `@jumpchoice/doc-parser` — service exists in `packages/server/src/services/jump-doc/` but no routes registered
> - `@jumpchoice/gm` — not yet implemented (see `ROADMAP.md` Phase 5)
> - `@jumpchoice/image-gen` — image gen exists inside `packages/server/`
>
> **For current state, see `ROADMAP.md`** which tracks actual `[x]`/`[ ]`
> status per phase.
>
> This file is preserved as planning context. Update it only when a described
