# Personal Reality Supplement (PRS) — Min-Max Analysis

> **SOURCE NOTE (read first):** The source file for this analysis is `docs/jumpchain-supplements/sources/personal-reality-supplement.html`, a Notion-export HTML of one player's Personal Reality checklist. The file captures the supplement's intro, the WP/CP rules, the UDS integration note, the BASICS, UTILITIES AND STRUCTURES, and PERSONAL REALITY FACILITIES sections in full, and the ITEMS & EQUIPMENT section. It does **not** contain the full STRUCTURES catalog, the MISC catalog, the Core Modes options beyond the default Incremental Core Mode, or the full Utilities roster that the canonical PRS document (SJ-Chan & Sonic0704) ships with. A complete tier list would require the canonical PRS source.
>
...
| Field | Value |
|---|---|
| Source | `docs/jumpchain-supplements/sources/personal-reality-supplement.html` (39,178 bytes, 670 lines, 1 body line with everything) |
...
| Currency | Warehouse Points (WP) — primary; CP → WP bridge at 50 CP : 2 WP |
...
The supplement's actual free starting bundle is: Cosmic Warehouse (a contained reality, "absolutely nothing else" inside, dim, empty, accessible from anywhere barring drawback/gauntlet blocks), Boxes and Boxes and Boxes (mundane storage containers only, no fiat, summoned from the "Box Office"), and a 80 × 80 × 10 m interior.
...
|---|---|---|---|
| UDS Chain-Drawback WP scalar | **10×** the original Quicksilver value | Per UDS drawback, per chain | "UDS Notes" intro |
| CP → WP bridge | 50 CP : 2 WP (25:1) | Per jump, after CP earned | "WP & CP" |
...
### 2.2 The CP bridge — last resort

50 CP : 2 WP is 25:1. At a typical jump's CP yield (1,000 CP default), that is 40 WP per jump — worse than the +50 passive you would have gotten from the Incremental Core Mode anyway. The CP bridge is only worth it if (a) you have a CP surplus you will never spend, (b) you bypassed PR acquisition past when Incremental Core Mode would have been retroactively granted, or (c) you are filling a specific WP gap before a major purchase. Generally: do not convert CP unless the WP unlocks something you cannot get any other way.

The source is explicit that **WP cannot be converted into CP**. Asymmetric. You can bleed CP into WP but not back.

### 2.3 The UDS pipeline — the meta channel

Quicksilver's original 150 WP Warehouse Supplement awarded points through specific chain-tagged drawbacks. PRS multiplies those by 10. The 10× scalar is a flat multiplier on the original WP yield, not on a fresh PRS-tuned number. This is critical: the design intent is that PRS scales with the original Warehouse as the unit, so a drawback that awarded 5 WP in Quicksilver's doc now awards 50 WP here. For a chain with multiple WP-granting UDS drawbacks, total WP can run into the hundreds per chain.
...
| Incremental Core Mode (10 × 50) | 500 |
| UDS pipeline (10-jump chain, 3 WP-granting chain drawbacks, original 15-30 WP each × 10) | ~450–900 |
| Per-jump Warehouse modifications (5 qualifying jumps, average +50 WP-equivalent) | ~250 |
| CP bridge (500 surplus CP) | 20 |
| **Estimated total** | **~1,720–2,170** |

The UDS pipeline and per-jump Warehouse modifications dominate. CP bridge is rounding error. The expected WP bank by jump 10 is therefore in the **1,500–2,500 range** under any reasonable build. That is the budget tier that all option rankings below should be evaluated against.
...
- **S** — must-buy, always relevant, scales with everything
- **A** — very strong, take in most builds
- **B** — solid, take if the budget or build theme supports it
- **C** — situational, niche
- **D** — skip unless flavor demands
- **F** — actively counterproductive

### §3.1 BASICS (free)

The BASICS section is the free baseline. All entries are free, so tiering is by "would you skip it?" — none of these cost WP, so the only question is whether there is a real reason not to take them.
...
| Tier | Cost | Option | Why |
|---|---|---|---|
| S | Free | **Starting Space (80 × 80 × 10 m)** | The interior is the floor. 64,000 m³ is a working volume — you do not have a Warehouse problem on day one. |
| S | Free | **Antibiotic Field** | Cross-jump biome/virome scrub. Solves the "plague carrier" failure mode that would otherwise make your PR a biohazard in a new setting. Medbay doesn't fix this — the medbay fixes current infections at jump's end, the field prevents new ones. |
...
| S | Free | **Environmentally Neutral** | Temperature, air composition, dual-species atmosphere support, gravity, dust. Free. The bar to clear is "would you ever *not* want this?" — no. |
| B | Free | **The Question of Shelving** | Industrial metal shelves, pallet-sized, numbered. Useful but trivially replicable. The free pallets are a starting convenience, not a permanent advantage — the supplement is upfront that you will need to buy more on your own. |
...
| B | Free | **A Week & A Button** | 7 Standard Earth Days of post-jump downtime per jump. The button to end it is a real feature. The downside — *"Dying in this period is still dying and will burn a 1-up from the jump you just finished if possible, then from the next jump if available"* — is the supplement's standard 1-up sink. Not a 1-up refund loophole. |
| A | 50 WP | **Basic Accommodations** | Bunk + locker + camp toilet per person. Scales to refugees, guests, anyone you admit. The supplement is explicit: *"someone need not be a companion for this to work"* and *"this can support up to a maximum of one person per day for every 4 cubic meters of space in your warehouse."* So 64,000 m³ supports 16,000 person-days of basic living. The camp toilet must be emptied manually — a real flavor cost. |
...
| Tier | Cost | Option | Why |
|---|---|---|---|
| B | Free | **Cleaning Supplies** | Infinite, summons to anyone allowed to reside, safe for complex life, *not* safe for viruses/bacteria/amoeba/household pests. Anti-pest layer that complements the Antibiotic Field. You cannot sell the supplies for chemicals — anti-cheese clause. |

### §3.5 GENERIC FIRST JUMP (excluded — different supplement)

The source includes a "GAINED FROM JUMP — GENERIC FIRST JUMP" section with three items (Bare Necessities, Franchise, Camera). These are not PRS options — they are Generic First Jump perks that one player has banked to their PR via the compatibility clause. They are excluded from the PRS tier list and are documented separately in the Generic First Jump analysis.

### §3.6 Source gap (canonical PRS-only options not in this source)

The source HTML does not capture the full STRUCTURES, MISC, or expanded UTILITIES catalogs. The canonical PRS document includes — per public references to SJ-Chan and Sonic0704's design — many additional options across:

- **STRUCTURES** — dedicated housing blueprints, defensive platforms, vehicle bays, research labs, secure vaults, training arenas. **Source Gap** for the full list and costs.
- **MISC** — companion quarters, exotic utilities, transport infrastructure, animal habitats. **Source Gap**.
- **Biosphere Supplement cross-reference** — only the one-line integration note is present. **Source Gap** on the actual Biosphere sub-options.
- **Multiple Core Modes** — only Incremental Core Mode is shown. The canonical doc reportedly has additional Core Modes (different starting budgets and scaling). **Source Gap**.
- **Extra-Modes** — the source mentions "Extra-Modes apply over Core modes, and there is no limit on how many of them can be picked" but does not list any. **Source Gap**.
...
The skew toward S is not a bias — it is a structural fact of the source. PRS is a foundation supplement, not a perk tree. Most options are either essential plumbing (S) or thematic additions (A/B). The expensive, build-defining decisions live in the **canonical options this source does not contain** — STRUCTURES and MISC.
...
Assume a 20-jump chain with 4 UDS chain-tagged drawbacks that grant WP, with original Quicksilver values of 15, 20, 25, and 30 WP (4 mid-range drawbacks). With the 10× scalar, those are 150, 200, 250, 300 — total 900 WP from the UDS pipeline alone.
...
- **Tier 1 budget: ~1,500 WP** — achievable by jump 20 with no UDS pipeline. This is the "casual completionist" tier and should cover most STRUCTURES and MISC options.
- **Tier 2 budget: ~2,500+ WP** — achievable by jump 20 with the UDS pipeline active. This is the "min-max" tier and unlocks whatever the canonical doc puts at the top end of the WP scale.
...
The source shows a worked example: a player with 500 starting + 50 earned (jump 1) = 550 total, who has spent 350 WP on Utilities/Structures and 100 WP on the Medical Bay, leaving 100 WP unspent (the "WP Spent: 100" line at the bottom of facilities). This matches 550 total − 450 spent = 100 remaining, consistent with the "CURRENT WP: 50" bank display — the source's CURRENT WP is 50, not 100. The discrepancy (probably a 50-WP rounding) is a Source Gap, but the order-of-magnitude is right: 500 + 50 (jump 1) − 450 (one full utilities bundle plus medbay) = 100 expected, source shows 50. Close enough; the source's worked example is internally consistent at the 50-WP level.
...
The UDS mechanic has Hiatus (suspend a drawback temporarily) and Revoke (drop a drawback permanently). If a UDS drawback that is the WP source is Hiatus'd or Revoked, the WP grant presumably lapses. The PRS source does not address this — the WP grant is a one-time chain-completion payout, not a perpetual income, so a Revoke would only matter if it happens before the chain completes. For pre-chain Revoke: the WP is forfeit. For post-chain Revoke: the WP has already been banked and is permanent.
...
| UDS pipeline, 10-jump chain, 3 WP-granting drawbacks (15 + 20 + 25 original, × 10) | 600 |
...
- **Access Key + Key Link** — the Key alone is "any door"; Key Link is "any door I have used the Key on". The multi-purchase clause means you can have N podiums at N key locations. This is the only access-control subsystem in the source, and it is mandatory.
- **Antibiotic Field + Medical Bay** — the field prevents new infections; the medbay clears existing ones at jump end. Stack both. The field is free, the medbay is 100 WP, and together they form the healthcare layer of any base.
- **Who's Got the Powa + Central Control** — electricity powers the smart control system. Central Control without power is a dark room with a terminal. The two together are the "smart base" bundle.
- **Basic Accommodations + Basic Nutrition + Entrance Hall** — civilian refugee support. The supplement text is explicit that anyone you admit gets quarters and food, not just companions. This is the build-block for any "haven" archetype.
- **Boxes and Boxes + Starting Space** — the mundane storage tier plus the interior volume. Without the space, the boxes have nowhere to live. Without the boxes, the space is empty.

### 6.2 Cross-supplement hooks (the META-IMPORTANT part)

- **UDS → PRS** (the WP pipeline) — see §5. This is the single largest cross-supplement synergy in the entire Jumpchain stack. UDS drawbacks that grant WP are the design's intended primary income for PRS.
- **Generic First Jump (GFJ) → PRS** — the source shows GFJ perks (Bare Necessities, Franchise, Camera) being banked to the PR. GFJ's income, lifestyle, and storytelling perks integrate with PR as content. For a 1-jump build, GFJ is the natural pre-PR acquisition.
- **Essential Body Mod / SB Body Mod → PRS** — both Body Mods are designed to be warehouse-relevant. The PRS compatibility clause ("compatible with all Warehouse modification options that can be found in various jumps") covers any Body Mod pocket-space as an expansion of the PR. Source Gap on the exact Body Mod mechanics, but the PRS side is permissive: anything that would have been a separate Warehouse perk stacks onto PR.
- **Biosphere Supplement (Fountain) ↔ PRS** — the source explicitly integrates: *"fully integrates with the Biosphere Supplement from the Fountain."* Biosphere is the canonical ecology layer; PRS is the canonical space layer; together they form a self-sustaining pocket reality.
- **CP → WP bridge (any CP-granting source)** — 50 CP : 2 WP. The bridge is bad as a primary path but useful as a CP-surplus sink. Any build that generates CP faster than it spends it (a UDS drawback that grants CP, a high-difficulty mode, a Jumper Background that boosts CP yield) can bleed the surplus into WP.

### 6.3 Antagonisms (within PRS)

- **Security System vs. many Keys** — the supplement text is explicit: *"A Key holder's Access cannot be limited by the Security System. Only hand out Keys to those you trust implicitly."* This is a soft antagonism: the more Keys you hand out, the more your security model is trust-based, not policy-based. The workaround is to never hand out Keys; use the Access Key + Key Link mechanism to maintain access yourself, and let guests use the basic accommodations (no Key required) for short stays.
- **Boxes and Boxes vs. WP bank** — Boxes are explicitly anti-fiat: *"These containers are in no way fiat-backed. They are completely normal."* If a build relies on WP to acquire special objects, do not expect Boxes to carry them. The two subsystems are independent.
...
- Cosmic Warehouse (free) + Starting Space (free) + The Loft (free) — interior volume.
- Who's Got the Powa (100 WP) + Central Control (100 WP) — power and control.
- Medical Bay (100 WP) + Antibiotic Field (free) — healthcare layer.
- Security System (free) + Access Key + Key Link (50 WP) — access control.
...
**Core picks (all source):**
- Access Key + Key Link (50 WP, multiple purchases) — N podiums at N prepared doors in the host world.
- Boxes and Boxes and Boxes (free) — fallback storage at the Entry Hall.
- Central Control (100 WP) — recall companions via Portal (when acquired) or after death.
- A Week & A Button (free) — downtime at the end of each jump for re-positioning.
- Basic Accommodations (50 WP) — if the mobile crew needs to rest at a key location.
...
**Core picks (all source):**
- Basic Accommodations (50 WP) + Basic Nutrition (50 WP) — the support layer.
- Entrance Hall, free + 1 additional (20 WP) — separation of public and private access.
- Security System (free) + Access Key + Key Link (50 WP) — tiered access for companions.
- Medical Bay (100 WP) — for the inevitable post-mission injuries.
- **Total: 270 WP.**
...
**Core picks (all source):**
- Starting Space (free) + The Loft (free) — volume and vertical build-out.
- Who's Got the Powa (100 WP) — power for industrial-scale work.
- Central Control (100 WP) — inventory and automation.
- Cleaning Supplies (free) — industrial cleaning.
- **Total: 200 WP.**
...
**Core picks (all source):**
- Access Key + Key Link (50 WP, multiple purchases) — the entire access model. Doors that only you know.
- Security System (free) — notification of unauthorized access attempts.
- The Loft (free) — vertical stacking in a small footprint.
- A Week & A Button (free) — downtime you can take anywhere.
...
### 8.1 Fortress Builder — "Smart Base" (jump 1 build)
...
### 8.2 Fortress Builder — "Layered Defense" (jump 10 build, UDS active)

**Pre-jump budget:** assumes 10-jump chain, 2 UDS chain-tagged WP-granting drawbacks (original 15 and 20, × 10 = 350 WP).
**Bank at jump 10:** 500 (base) + 500 (10 × 50 passive) + 350 (UDS) = **1,350 WP**.

**Spend profile:**
- Layer 1 (Essentials, 350 WP) — same as 8.1.
- Layer 2 (Healthcare, already in Layer 1).
- Layer 3 (Companion support, 100 WP) — Basic Accommodations (50 WP) + Basic Nutrition (50 WP).
- Reserve (900 WP) — banked for canonical STRUCTURES (Source Gap) options.
- **Total: 450 WP spent. Bank: 900 WP reserved.**

### 8.3 Mobile Stronghold — "The Prepared Doorkeeper" (jump 5 build)

**Pre-jump budget:** 500 (base) + 250 (5 × 50 passive) = 750 WP. No UDS active.
**Spend profile:**
- Key Link × 3 (150 WP) — three podiums at three prepared doors in the current host world.
- Central Control (100 WP) — companion recall and door automation.
- Basic Accommodations (50 WP) — for the mobile crew.
- **Total: 300 WP. Bank: 450 WP.**

### 8.4 Mobile Stronghold — "The Network" (jump 15 build, UDS active)

**Pre-jump budget:** 500 + 750 + 700 (3 UDS drawbacks, 10+15+20 original × 10) = 1,950 WP.
**Spend profile:**
- Layer 1 (Essentials, 350 WP) — same as Fortress Builder baseline.
- Layer 2 (Network, 250 WP) — Key Link × 5 (250 WP).
- Layer 3 (Companion support, 100 WP) — Basic Accommodations + Nutrition.
- Reserve (1,250 WP) — banked for transport infrastructure (Source Gap) and STRUCTURES.
- **Total: 700 WP spent. Bank: 1,250 WP.**

### 8.5 Companion Haven — "Refugee Center" (jump 3 build)

**Pre-jump budget:** 500 + 150 = 650 WP.
**Spend profile:**
- Basic Accommodations (50 WP) + Basic Nutrition (50 WP) = 100 WP.
- Medical Bay (100 WP) — for incoming wounded.
- Entrance Hall, free + 1 additional (20 WP) — separation of public intake from private PR.
...
### 8.6 Resource Extraction Hub — "The Workshop" (jump 7 build)

**Pre-jump budget:** 500 + 350 = 850 WP.
**Spend profile:**
- Who's Got the Powa (100 WP) + Central Control (100 WP) = 200 WP.
- Multiple Entrance Halls (3 × 20 = 60 WP) — one per workshop entrance.
- Basic Nutrition (50 WP) — for the crew.
- **Total: 310 WP. Bank: 540 WP.**

### 8.7 Stealth Sanctuary — "The Hidden Door" (jump 2 build, UDS active)

**Pre-jump budget:** 500 + 100 + 150 (1 UDS drawback, 15 original × 10) = 750 WP.
**Spend profile:**
- Key Link × 2 (100 WP) — two prepared doors in two different regions of the host world.
- Security System (free) — for notification.
- Medical Bay (100 WP) — for self-recovery.
...
This is a loan mechanic. You can start a WP purchase by paying a fraction of the CP cost in jump N, and the WP-granted option is not delivered until the full CP is paid across jumps. The source does not specify the interest rate (there is none — flat pay-as-you-go) or the late-payment penalty (there is none — you simply don't get the option until paid).

Ruling: zero-interest installment plan. Useful for high-cost canonical STRUCTURES options (Source Gap) where the full CP cost might exceed a single jump's surplus. No reason not to use it for anything that can be paid in 2–3 jumps.
...
- Can the PR be entered by the Jumper when the Jumper is in *any* reality, including post-chain? The source says "no matter where you go (unless blocked by a drawback or gauntlet) you'll always be able to access it" — this is unconditional, suggesting the PR is accessible from any reality, not just in-chain.
...
These are **soft Source Gaps** — the supplement is mostly silent on persistence because the design intent (PRS is permanent) is taken for granted. A complete analysis would confirm against the canonical PRS doc.
...
- **Cleaning Supplies are not safe for viruses, bacteria, amoeba, or household pests.** This is a feature, not a bug — the Antibiotic Field handles the disease vector, and Cleaning Supplies handles the pest vector.
...
PRS is "not compatible with any of the alternatives that replace one of these" — meaning you cannot take PRS and Fortress of Jumper. PRS is "compatible with all Warehouse modification options that can be found in various jumps" — meaning you can stack per-jump Warehouse perks onto PRS. The distinction is between **replacement** (mutually exclusive) and **modification** (stackable). A build that takes PRS cannot also take Fortress of Jumper; a build that takes PRS can still take the "Warehouse has a hangar" perk from a jump that offers it.
...
### 10.1 The 10× WP scalar — the headline number

The original Quicksilver Warehouse Supplement awarded up to 150 WP over a chain. PRS's Incremental Core Mode awards 500 WP base + 50 per jump. By jump 20, PRS has awarded 1,500 WP from the Core Mode alone — **10× the original's max**. By the time UDS pipeline and per-jump Warehouse modifications are added, the WP bank can reach 2,500+ WP by jump 20 — **16×+ the original's max**.

This is not subtle power creep. It is an order of magnitude.

### 10.2 The integration guarantee — the structural change
...
### 10.3 The Compatibility with per-jump Warehouse mods — the multiplier
...
### 10.4 The Biosphere Supplement integration — the cross-stack effect
...
PRS is at minimum **10×** more powerful than the original Cosmic Warehouse by WP bank alone. With UDS pipeline, per-jump mods, and Biosphere integration, the effective power is closer to **20–50×**. The only Warehouse-replacement alternative that competes is Fortress of Jumper, and the source explicitly excludes the combination — so the design intent is that PRS *is* the modern warehouse, full stop.

The power creep is intentional. The 10× scalar on the UDS pipeline is the design's signature: this is a supplement that wants you to be able to afford everything it offers, and the UDS is the funding mechanism.

### 10.6 What PRS does NOT power-creep over

- **Companion power levels** — PRS does not buff companions. Companions benefit from the same free accommodations, nutrition, medical, and security as the Jumper, but they do not gain combat abilities or perks from PR ownership.
- **Jumper combat power** — PRS has no combat options in the source. The Medical Bay treats wounds, but does not grant defenses. STRUCTURES (Source Gap) likely includes defensive platforms, but the source does not confirm.
- **Cross-jump lore/identity perks** — PRS does not touch the Jumper's background, identity, or skillset. It is a Warehouse replacement, not a Jumper replacement.
- **CP yield** — the CP→WP bridge is one-way. PRS does not increase CP income.
...
- The 10× scalar is the design's load-bearing rule. Any build that takes PRS without engaging the UDS pipeline is leaving ~38% of the WP budget on the table over a 20-jump chain.
...
- **UDS** funds the WP pipeline (10× scalar on chain-tagged drawbacks).
...
| G6 | In-PR WP gain | "Some options below give you a much better rate of return" — which options? | Affects optimal spend profile. |
...
| G12 | Spec upgrade options (Basic → Deluxe) | PRS source has no upgrade paths shown. | Source may simply not include them. |
...
```
PRS — Personal Reality Supplement
...
## Appendix C — What this analysis does not cover (and why)

- The full STRUCTURES catalog — Source Gap G3.
- The full MISC catalog — Source Gap G4.
- The full Extra-Modes list — Source Gap G2.
- Per-jump Warehouse modifications — lives in per-jump docs, not in PRS.
- The Biosphere Supplement's option list — lives in the Biosphere doc.
- UDS-specific WP-granting drawbacks — lives in the UDS analysis (in flight).
- A specific per-jump budget worked example beyond jump 20 — the source does not provide a canonical long-chain sample.
