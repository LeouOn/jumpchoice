...
Would you like to retry?" IDYES retryInstall IDNO skipRetryInstall
  ...
  ${If} $PNPM_RUNNER == "corepack"
    nsExec::ExecToLog 'cmd /c corepack pnpm@${PNPM_VERSION} --filter @jumpchoice/shared build'
    Pop $0
    ${If} $0 == 0
      nsExec::ExecToLog 'cmd /c corepack pnpm@${PNPM_VERSION} --filter @jumpchoice/server --filter @jumpchoice/client --parallel run build'
      Pop $0
    ${EndIf}
  ${EndIf}
  ${If} $PNPM_RUNNER == "npx"
