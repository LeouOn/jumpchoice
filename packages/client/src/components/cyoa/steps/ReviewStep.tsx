  ...
  if (entries.length === 0) {
    return (
      <div className="flex flex-col items-center gap-2 py-12 text-center">
        ...
        <p className="text-sm text-[var(--muted-foreground)]">No extracted data to review yet.</p>
        <p className="text-xs text-[var(--muted-foreground)]">Run extraction on the Extract tab first.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--card)] px-3 py-2.5">
        ...
        {totalBudget > 0 && (
          <span className="rounded bg-[var(--accent)]/60 px-1.5 py-0.5 text-[10px] text-[var(--foreground)]">
            {totalBudget} pt budget
          </span>
        )}
        {categories.length > 0 && (
          <span className="text-[10px] text-[var(--muted-foreground)]">
            {categories.length} categor{categories.length !== 1 ? "ies" : "y"}: {categories.join(", ")}
          </span>
        )}
        <span className="text-[10px] text-[var(--muted-foreground)]">{pagesWithExtractions} page{pagesWithExtractions !== 1 ? "s" : ""}</span>
        {warnings.length > 0 && (
        ...
        {hasEdits && (
          <span className="rounded bg-[var(--primary)]/15 px-1.5 py-0.5 text-[10px] font-medium text-[var(--primary)]">
            Edited
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-1">
        ...
        >
          {expandedPages.size === entries.length ? "Collapse All" : "Expand All"}
        </button>
      </div>
        ...
        <div className="flex flex-col gap-1 rounded-md border border-amber-500/20 bg-amber-500/5 px-3 py-2">
          {warnings.map((w, i) => (
            <div key={i} className="flex items-start gap-1.5 text-[10px] text-amber-400">
              ...
              <span>
                <span className="font-medium">Page {w.pageNumber}:</span> {w.text}
              </span>
            </div>
          ))}
        </div>
                  ...
                  <div className="border-t border-[var(--border)] px-3 py-2">
                    ...
                    {choices.length === 0 ? (
                      <p className="py-2 text-center text-[10px] text-[var(--muted-foreground)]">
                        No choices extracted from this page.
                      </p>
                    ) : (
                        ...
                        {choices.map((choice, ci) => (
                          ...
                          >
                            ...
                            <div className="flex items-start gap-2">
                              ...
                              />
                              {choice.stealth && (
                                <span className="mt-4 shrink-0 rounded bg-purple-500/15 px-1.5 py-0.5 text-[10px] font-medium text-purple-400">
                                  Stealth
                                </span>
                              )}
                            </div>
      ...
      <div className="flex items-center justify-between">
        <p className="text-[10px] text-[var(--muted-foreground)]">
          {editMode === "structured"
            ? "Edit any field inline — changes are saved locally until you approve."
            : "Edit the raw JSON, then approve to save."}
        </p>
        <button
