        ...
        {noConnections ? (
          <div className="flex items-start gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2">
            ...
            <div className="text-xs text-amber-400">
              <p className="font-medium">No API connections configured</p>
              <p className="mt-0.5">
                You need at least one LLM connection with vision support (OpenRouter, Zhipu, or OpenAI).{" "}
                Add one in the Connections panel, then come back here.
              </p>
            </div>
          </div>
        ) : (
      ...
      >
        {isExtracting ? (
          <>
            ...
            Extracting… ({completedCount}/{totalCount} pages)
          </>
        ) : (
          <>
            <Zap className="h-4 w-4" />
            {allExtracted ? "Extraction Complete" : "Start Extraction"}
          </>
        )}
      </button>
      ...
      {isExtracting && totalCount > 0 && (
        <div className="space-y-1">
          <div className="flex items-center justify-between text-[10px] text-[var(--muted-foreground)]">
            <span>Processing page {Math.min(completedCount + 1, totalCount)} of {totalCount}</span>
      ...
      {images.length > 0 && (
        <div className="flex flex-col gap-2">
          <h3 className="text-xs font-medium text-[var(--muted-foreground)]">
            Image Progress ({completedCount}/{images.length})
          </h3>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <span className="text-sm text-[var(--foreground)]">Page {img.pageNumber}</span>
                    ...
                    <div className="flex shrink-0 items-center gap-1.5">
                      ...
                      {isPending && (
                        <span className="text-[10px] text-[var(--muted-foreground)]">waiting</span>
                      )}
                      {isProcessing && (
                        <span className="text-[10px] text-[var(--primary)] animate-pulse">processing</span>
                      )}
                    </div>
                  </div>
                );
