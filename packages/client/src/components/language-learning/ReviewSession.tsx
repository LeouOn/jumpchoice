  ...
  if (!current) {
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-4 p-8">
        ...
        <h3 className="text-lg font-semibold text-[var(--foreground)]">Session Complete!</h3>
        <p className="text-sm text-[var(--muted-foreground)]">
          You reviewed {reviewedCount} of {items.length} cards.
        </p>
        <button
          onClick={() => {
            endReviewSession();
      ...
      <div className="w-full rounded-xl bg-[var(--card)] p-6 ring-1 ring-[var(--border)] shadow-sm">
        <div className="text-center">
          ...
          {current.lemma !== current.surface && (
            <div className="mt-1 text-xs text-[var(--muted-foreground)]">lemma: {current.lemma}</div>
          )}
        </div>
          ...
          <>
            <div className="mt-4 border-t border-[var(--border)] pt-4 text-center">
              ...
              {current.contextSentence && (
                <div className="mt-2 text-xs italic text-[var(--muted-foreground)]">
                  &ldquo;{current.contextSentence}&rdquo;
                </div>
              )}
            </div>
